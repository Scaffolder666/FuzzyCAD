...
// Proposed Hole
// Uses Onshape's high-level extrude feature for REMOVE so the boolean
// is explicitly scoped to the intended body.
...
export const fuzzycadProposedHole = defineFeature(function(context is Context, id is Id, definition is map)
    ...
    }
    {
        ...
        // Dashed centerline extends slightly beyond the cut.
        drawDashedReferenceLine(
            context,
            id + "holeAxis",
            cutPlane.origin - direction * (8 * millimeter),
            cutPlane.origin + direction * (definition.depth + 10 * millimeter)
        );
        ...
        drawEngineeringLinearArrow(
            ...
            id + "holeArrow",
            cutPlane.origin,
            cutPlane.origin + direction * definition.depth,
