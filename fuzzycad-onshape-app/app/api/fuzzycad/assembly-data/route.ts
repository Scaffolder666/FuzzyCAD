/**
 * Combined assembly-data endpoint.
 *
 * Calls getCachedAssembly once and returns both the assembly summary
 * (previously /api/fuzzycad/assembly-summary) and the relationship graph
 * (previously /api/fuzzycad/relationship-graph) in a single response.
 *
 * This saves one Onshape API call on cache-miss and eliminates a full
 * client→server round-trip relative to calling the two endpoints separately.
 */
...
function summarizeOccurrences(rootAssembly: UnknownRecord) {
  ...
  return occurrences.slice(0, 30).map((occurrence, index) => {
    ...
    return {
      ...
      translation:
        transform.length === 16
          ? { x: transform[3], y: transform[7], z: transform[11] }
          : null,
      transform,
    };
  });
...
function summarizeFeatures(rootAssembly: UnknownRecord) {
  ...
  return features.slice(0, 80).map((feature, index) => {
    if (!isRecord(feature)) return { index, rawType: typeof feature };
...
function summarizeInstances(rootAssembly: UnknownRecord) {
  ...
  return instances.slice(0, 80).map((instance, index) => {
    if (!isRecord(instance)) return { index, rawType: typeof instance };
...
function buildSummary(rootAssembly: UnknownRecord) {
  return {
    counts: {
...
function buildInstances(
  ...
  for (const block of blocks) {
    asArray(block.record.instances).forEach((item, index) => {
      const record = isRecord(item) ? item : {};
...
function buildSameSourceGroups(instances: InstanceNode[]): SameSourceGroup[] {
  ...
  for (const instance of instances) {
    ...
    if (key === "unknownDocument:unknownElement:unknownPart") continue;
...
function buildWarnings(
  ...
  // Suppress unused-variable warning — rootAssembly is passed for parity with
  // the standalone relationship-graph route.
  void rootAssembly;
...
export async function GET(req: NextRequest) {
  ...
  const assemblyResult = await getCachedAssembly({
    ...
    assemblyElementId,
    accessToken,
    route: "/api/fuzzycad/assembly-data",
    force,
  });
  ...
  const summary = buildSummary(rootAssembly);
  const graph = buildGraph(assemblyResult.data, {
    documentId,
    workspaceId,
    assemblyElementId,
    server,
