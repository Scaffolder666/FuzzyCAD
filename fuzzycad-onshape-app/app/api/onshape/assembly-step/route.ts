...
function makeCacheKey(input: {
  ...
  return [input.server, input.documentId, input.workspaceId, input.assemblyElementId].join("|");
}
...
async function getTranslationStatus(href: string, accessToken: string): Promise<UnknownRecord> {
  const res = await onshapeFetch(
    ...
    { route: "/api/onshape/assembly-step", operation: "get-translation-status" },
  );
...
/**
 * Exports an assembly to STEP via Onshape's general-purpose assembly
 * translation endpoint (not a dedicated /export/step sub-path — that's
 * only confirmed to exist for Part Studios/Parts; assemblies go through
 * /translations with formatName). Mirrors the async
 * export-then-poll-then-download shape of assembly-gltf/route.ts, minus
 * the multi-file zip unpacking (STEP is always a single file) and the
 * blob-snapshot caching (not yet needed here).
 */
export async function GET(req: NextRequest) {
  ...
  const translationsEndpoint = `${server}/api/assemblies/d/${documentId}/w/${workspaceId}/e/${assemblyElementId}/translations`;

  const cacheKey = makeCacheKey({ server, documentId, workspaceId, assemblyElementId });
  ...
  const exportRes = await onshapeFetch(
    ...
    { route: "/api/onshape/assembly-step", operation: "start-step-export" },
  );
    ...
    return NextResponse.json(
      {
        ...
        ok: false,
        error: "Failed to start assembly STEP export",
        details: errorData,
      },
      { status: exportRes.status },
    );
  ...
  const downloadRes = await onshapeFetch(
    ...
    { route: "/api/onshape/assembly-step", operation: "download-step-result" },
  );
